 ...
 * @notice Core on-chain verifier for the TrustLayer protocol.
 *
 * Verifies that an ACP Provider:
 *  1. Actually fetched data from a trusted real-world HTTPS endpoint
 *  2. Fed that exact data (by hash) into a trusted LLM API
 *  3. The Deliverable Memo reflects the true LLM output
 *
 * All proofs are generated by Primus zkTLS running inside Phala TEE.
 *
 * Deployed on Base mainnet.
 * Primus verifier: 0xCE7cefB3B5A7eB44B59F60327A53c9Ce53B0afdE
 */
contract TrustLayerVerifier is ITrustLayer, Ownable {
    ...
    function verifyProofBundle(
        ...
        // Note: TypeScript uses SHA256-based rolling hash; Solidity uses keccak256.
        // The SDK encodes chainHash as keccak256 for on-chain use.
        require(
